...
def copy_folder_with_permissions(root_dir: Path, source_dir: Path, target_dir: Path):
    ...
    :param root_dir: path to some previous folder of the given source
    :param source_dir: path to a folder to copy
    :param target_dir: path to place the copied folder
    :return:
    """
    folder_content = {}
    ...
    def walk_callback(root, subdirs, files):
        for file_in in files:
            file_path_in = Path(root, file_in)
            relative_path = file_path_in.relative_path(root_dir.path)

            with open(file_path_in.path, "rb") as f_in:
                folder_content[relative_path] = (file_path_in.state().st_mode, f_in.read())
                f_in.close()

    source_dir.walk(walk_callback, recursive=True)

    for file in folder_content.keys():
        file_path = target_dir.copy().join(file)
        if not Path(file_path.dirname()).exists():
            Path(file_path.dirname()).make(override=False, ignore_errors=True, directory=True)

        with open(file_path.path, "wb+") as f:
            st_mode, file_content = folder_content[file]
